...
static void write_u64(std::vector<uint8_t>& b, size_t off, uint64_t v) {
    for (int i = 0; i < 8; i++) {
...
std::optional<std::vector<uint16_t>> Linker::linkObjects(const std::vector<std::vector<uint8_t>>& objects, LinkError* err) {
    ...
    struct ObjectContext {
        ...
        struct SymEntry { uint32_t name; uint8_t info; uint16_t shndx; uint64_t value; };
    ...
    for (const auto& file : objects) {
        ...
        for (uint16_t si = 0; si < e_shnum; ++si) {
            ...
            uint32_t type = read_u32(file, sh_base + 4);
        ...
        if (symtab_offset && symtab_size && symtab_entsize == 24 && strtab_offset && strtab_size) {
            ...
            for (uint64_t i = 0; i < entries; ++i) {
                ...
                if (off + symtab_entsize > file.size()) break;
                uint32_t name = read_u32(file, off);
                if (name >= strtab_size) continue;
                uint8_t info = file[off + 4];
                uint16_t shndx = read_u16(file, off + 6);
                uint64_t value = read_u64(file, off + 8);
                uint64_t siz = read_u64(file, off + 16);

                // Skip null symbol entry (index 0)
                if (i == 0) continue;

                const char* sym_name = (const char*)&file[strtab_offset + name];
                ctx.sym_names.emplace_back(sym_name);
                ctx.syms.push_back({ name, info, shndx, value });
            }
        }
    ...
    for (auto& ctx : objs) {
        for (size_t i = 0; i < ctx.syms.size(); ++i) {
            auto& s = ctx.syms[i];
            std::string name = ctx.sym_names[i];
            bool defined = (s.shndx != 0);
            if (defined) {
                uint64_t abs_addr = text_base + s.value;
                global_symbols[name] = abs_addr;
            }
        }
        text_base += ctx.text.size;
    }

    // Start concatenating .text and apply relocations.
    ...
    for (auto& ctx : objs) {
        ...
        // apply relocations for this object relative to text_base
        for (auto& r : ctx.relocs) {
            ...
            if (r.sym == 0 || r.sym > ctx.syms.size()) {
                if (err) err->message = "Invalid symbol index in relocation";
                return std::nullopt;
            }
            std::string sym_name = ctx.sym_names[r.sym - 1];
            if (global_symbols.find(sym_name) == global_symbols.end()) {
                if (err) err->message = "Undefined symbol in relocation: " + sym_name;
                return std::nullopt;
            }
            uint64_t sym_addr = global_symbols[sym_name];
            uint64_t patch_addr = text_base + r.offset;
            if (r.type == 1) {
                // PCREL6
