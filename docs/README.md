...
- Canonical Linux boot helper: `little64 boot run`
	...
	- Default launch is `--launch=bootrom` with `--integrated-rom=litex-bios`, which runs through the LiteX BIOS bootrom path using a BIOS-compatible SD image.
	...
	- `--launch=direct` switches back to the preloaded post-firmware handoff path, and `--integrated-rom=stage0` keeps the legacy shared SD loader when you explicitly want it.
	...
	- Use `--mode=rsp` to launch the bootrom-first RSP debug server.
...
- Arty hardware build/program helper: `./.venv/bin/little64 hdl arty-build`
	...
	- The default staged bootrom now comes from the repo-local LiteX BIOS port and emits a LiteX BIOS SD image; use `--no-use-litex-bios` to fall back to the shared `target/c_boot/litex_sd_boot.c` loader.
	- The Arty helper now defaults to the native Adafruit 4-bit SDIO breakout preset on `IO34..40` in the breakout's physical header order `CLK, D0, CMD, D3, D1, D2, DET`; its native card-detect input defaults active-low and can be overridden with `--[no-]sdcard-det-active-low`, while the older SPI Arduino preset on `IO30..33` plus PMOD mappings remain available via `--sdcard-mode spi`.
	- Current native Arty bootrom status: stage-0 now keeps filesystem reads on the 4-bit native LiteSDCard path by default. If the earlier framing bug reappears on hardware, `target/c_boot/litex_sd_boot.c` still has an explicit `SDCARD_NATIVE_FORCE_1BIT_FILESYSTEM_IO` fallback knob for reverting to the older 1-bit filesystem path.
	- The Arty helper preloads the selected bootrom build into the integrated boot ROM; Linux-side SPI-SD rootfs integration is still separate work.
...
- SD boot artifact builder used by LiteX smoke and emulator LiteX boots: `./.venv/bin/little64 sd build`
	- `--machine litex --output-dir PATH` resolves the default LiteX kernel, generates DTS/DTB internally, and writes a machine-matched bootrom plus SD artifact set into `PATH`.
