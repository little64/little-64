...
class RspController:
    ...
    def start(self, *, required_paths: dict[str, pathlib.Path], trace_path: str | None = None) -> int:
        ...
        if proc.poll() is not None:
            ...
            if self.failure_tail_lines > 0:
                try:
                    tail = self.logfile.read_text(encoding="utf-8", errors="ignore").splitlines()[-self.failure_tail_lines:]
                    if tail:
