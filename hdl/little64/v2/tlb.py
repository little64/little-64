...
class Little64V2TLB(Elaboratable):
    def __init__(self, *, entries: int = 64, address_width: int = 64, generation_bits: int = 8) -> None:
        if entries < 2 or entries & (entries - 1):
            raise ValueError('Little64V2TLB entries must be a power of two and at least 2')
        if generation_bits < 1:
            raise ValueError('Little64V2TLB generation_bits must be at least 1')
