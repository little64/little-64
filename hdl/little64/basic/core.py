...
class Little64Core(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        a = Signal(64)
        b = Signal(64)
        rd = instruction_rd(self.current_instruction)
        rs1 = instruction_rs1(self.current_instruction)
        gp_opcode = instruction_gp_opcode(self.current_instruction)
        top2 = instruction_top2(self.current_instruction)
        top3 = instruction_top3(self.current_instruction)
        ls_opcode = instruction_ls_opcode(self.current_instruction)
        ls_offset2 = instruction_ls_offset2(self.current_instruction)
        ...
        ls_pc_rel6 = sign_extend(self.current_instruction[4:10], 6, 64)
        ls_pc_rel10 = sign_extend(self.current_instruction[:10], 10, 64)
            ...
            with m.Case(CoreState.EXECUTE):
                with m.If(top2 == 0b10):
                    ...
                    with m.Switch(shift):
                        ...
                        with m.Case(3):
                            sign_extended_imm = sign_extend(Cat(Const(0, 24), imm8), 32, 64)
                ...
                with m.Elif(top3 == 0b110):
                    ...
                    with m.Switch(gp_opcode):
                        with m.Case(GPOpcode.ADD):
                            ...
                            m.d.sync += [
                                self.flags.eq(flag_value(sum_value[:64], sum_value[64])),
                                self.register_file[15].eq(post_increment_pc),
                                self.state.eq(CoreState.FETCH_TRANSLATE),
                            ]
                        with m.Case(GPOpcode.SUB):
                            ...
                            m.d.sync += [
                                self.flags.eq(flag_value(sub_value, b > a)),
                                self.register_file[15].eq(post_increment_pc),
                                self.state.eq(CoreState.FETCH_TRANSLATE),
                            ]
                        ...
                        with m.Case(GPOpcode.AND):
                            ...
                            write_reg(rd, result)
                            m.d.sync += [
                                self.flags.eq(flag_value(result, Const(0, 1))),
                                self.register_file[15].eq(post_increment_pc),
                                self.state.eq(CoreState.FETCH_TRANSLATE),
                            ]
                ...
                with m.Elif(top3 == 0b111):
                    jump_offset = sign_extend(self.current_instruction[:13], 13, 64) << 1
                    m.d.comb += self.commit_valid.eq(1)
                    m.d.sync += [
