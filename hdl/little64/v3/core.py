...
class Little64V3Core(Elaboratable):
    """Experimental V3 core with explicit parallel pipeline stages.

    Fetch, decode, execute, memory, and retire now run as distinct submodules.
    The top level keeps only architected state, pipeline registers, fetch
    steering, and hazard/backpressure policy.
    """

    def __init__(self, config: Little64CoreConfig | None = None) -> None:
        self.config = config or Little64CoreConfig(core_variant='v3')
    ...
    def elaborate(self, platform):
        ...
        decode_stage = Little64V3DecodeStage(self.regs, self.flags)
        ...
        def clear_instruction_latches_sync():
            return [
                self.current_instruction.eq(0),
                self.execute_instruction.eq(0),
            ]

        def clear_memory_stage_sync():
            return [
                memory.valid.eq(0),
                memory.request_started.eq(0),
                memory.virtual_addr.eq(0),
                memory.addr.eq(0),
                memory.phys_valid.eq(0),
                memory.width_bytes.eq(0),
                memory.write.eq(0),
                memory.store_value.eq(0),
                memory.reg_write.eq(0),
                memory.reg_index.eq(0),
                memory.next_pc.eq(0),
                memory.commit_pc.eq(0),
                memory.fault_pc.eq(0),
                memory.post_reg_write.eq(0),
                memory.post_reg_index.eq(0),
                memory.post_reg_value.eq(0),
                memory.post_reg_delta.eq(0),
                memory.post_reg_use_load_result.eq(0),
                memory.chain_store.eq(0),
                memory.chain_store_addr.eq(0),
                memory.chain_store_use_load_result.eq(0),
                memory.chain_store_value.eq(0),
                memory.flags_write.eq(0),
                memory.flags_value.eq(self.flags),
                memory.set_reservation.eq(0),
                memory.reservation_addr.eq(0),
            ]

        def clear_retire_stage_sync():
            return [
                retire.valid.eq(0),
        ...
        if self.dcache is not None:
            m.d.comb += [
                self.dcache.lookup_addr.eq(memory.addr),
        ...
        m.d.comb += [
            decode_stage.instruction.eq(self.current_instruction),
            decode_stage.execute_valid.eq(execute_valid & ~execute_stage.outputs.memory_start),
            ...
            decode_stage.pending_load_memory_index.eq(memory.reg_index),
            ...
            decode_stage.pending_write_memory_valid.eq(memory.valid & memory.post_reg_write),
            decode_stage.pending_write_memory_index.eq(memory.post_reg_index),
            ...
            memory_stage.valid.eq(memory.valid & memory.phys_valid & ~cached_memory_response),
            memory_stage.request_started.eq(memory.request_started),
            memory_stage.addr.eq(memory.addr),
            memory_stage.width_bytes.eq(memory.width_bytes),
            memory_stage.write.eq(memory.write),
            memory_stage.store_value.eq(memory.store_value),
            memory_stage.reg_write.eq(memory.reg_write),
            memory_stage.reg_index.eq(memory.reg_index),
            memory_stage.next_pc.eq(memory.next_pc),
            memory_stage.commit_pc.eq(memory.commit_pc),
            memory_stage.flags_write.eq(memory.flags_write),
            memory_stage.flags_value.eq(memory.flags_value),
            memory_stage.set_reservation.eq(memory.set_reservation),
            memory_stage.reservation_addr.eq(memory.reservation_addr),
            memory_stage.post_reg_write.eq(memory.post_reg_write),
            memory_stage.post_reg_index.eq(memory.post_reg_index),
            memory_stage.post_reg_value.eq(memory.post_reg_value),
            memory_stage.post_reg_delta.eq(memory.post_reg_delta),
            memory_stage.post_reg_use_load_result.eq(memory.post_reg_use_load_result),
            memory_stage.chain_store.eq(memory.chain_store),
            memory_stage.chain_store_addr.eq(memory.chain_store_addr),
            memory_stage.chain_store_use_load_result.eq(memory.chain_store_use_load_result),
            memory_stage.chain_store_value.eq(memory.chain_store_value),
            memory_stage.lsu_request_ready.eq(self.lsu.request_ready & ~vector_load_active & ~walk_active),
            memory_stage.lsu_response_valid.eq(self.lsu.response_valid & ~vector_load_active & ~walk_active),
        ...
        m.d.comb += [
            ...
            pending_memory_load_valid.eq(memory.valid & ~memory.write & memory.reg_write),
            execute_redirect.eq(
                execute_valid &
            ...
            execute_flush_younger.eq(
                execute_valid &
                (execute_redirect | execute_stage.outputs.halt | execute_stage.outputs.trap | execute_stage.outputs.lockup)
            ),
            ...
            store_overlaps_reservation.eq(
                ...
                memory_to_retire &
                memory.write &
                (memory.width_bytes != 0) &
                (memory.virtual_addr <= reservation_end) &
            ...
            memory_translate_phys.eq(Mux(paging_enabled, tlb_lookup_paddr, memory.virtual_addr)),
            memory_fault.eq(
                memory_translate_ready &
                (paging_enabled & (~is_canonical39(memory.virtual_addr) | ((self.special_regs.page_table_root_physical & 0xFFF) != 0)))
            ),
            memory_fault_cause.eq(Mux(~is_canonical39(memory.virtual_addr), TrapVector.PAGE_FAULT_CANONICAL, TrapVector.PAGE_FAULT_RESERVED)),
            memory_fault_addr.eq(memory.virtual_addr),
            memory_fault_access.eq(Mux(memory.write, ACCESS_WRITE, ACCESS_READ)),
            memory_fault_pc.eq(memory.fault_pc),
            memory_fault_aux.eq(
                Mux(
                    ~is_canonical39(memory.virtual_addr),
            ...
            walk_fault_pc.eq(Mux(walk_resume_kind == WALK_RESUME_MEMORY, memory.fault_pc, walk_virtual_addr)),
            walk_fault_aux.eq(
                Mux(
                    (walk_pte & PTE_V) == 0,
            ...
            fetch_accept.eq(frontend_accept & (~decode_valid | decode_advance)),
            next_state.eq(V3PipelineState.FETCH),
            ...
            self.special_regs.core_trap_write.eq((self.state == V3PipelineState.RESET) | trap_request),
            self.special_regs.core_trap_cause_data.eq(
                Mux(
                    self.state == V3PipelineState.RESET,
                    0,
            ...
            self.special_regs.core_trap_pc_data.eq(Mux(self.state == V3PipelineState.RESET, 0, trap_pc)),
            self.special_regs.core_trap_aux_data.eq(trap_aux),
            self.lsu.request_valid.eq(Mux(walk_active, walk_request_valid, Mux(vector_load_active & vector_phys_valid, vector_request_valid, memory_stage.lsu_request_valid))),
            self.lsu.request_addr.eq(Mux(walk_active, walk_pte_addr, Mux(vector_load_active & vector_phys_valid, vector_load_addr, memory_stage.lsu_request_addr))),
            self.lsu.request_width_bytes.eq(Mux(walk_active | (vector_load_active & vector_phys_valid), 8, memory_stage.lsu_request_width_bytes)),
            self.lsu.request_write.eq(Mux(walk_active | (vector_load_active & vector_phys_valid), 0, memory_stage.lsu_request_write)),
            self.lsu.request_store_value.eq(Mux(walk_active | (vector_load_active & vector_phys_valid), 0, memory_stage.lsu_request_store_value)),
        ]

        m.d.comb += [
            mem_byte_offset.eq(memory.addr[0:3]),
            mem_base_sel.eq(Mux(memory.width_bytes == 1, 0x01,
                            Mux(memory.width_bytes == 2, 0x03,
                            Mux(memory.width_bytes == 4, 0x0F,
                            Mux(memory.width_bytes == 8, 0xFF, 0x00))))),
            mem_single_line_access.eq((memory.addr[0:3] + memory.width_bytes) <= 8),
            dcache_cacheable_load.eq(Const(1 if self.dcache is not None else 0, 1) & memory.valid & memory.phys_valid & ~memory.write & ~memory.request_started & mem_single_line_access),
            dcache_cacheable_fill.eq(Const(1 if self.dcache is not None else 0, 1) & memory.valid & memory.phys_valid & ~memory.write & (memory.width_bytes == 8) & (memory.addr[0:3] == 0)),
            dcache_store_update_valid.eq(0),
            dcache_store_update_addr.eq(memory.addr),
            dcache_store_update_sel.eq(mem_shifted_sel),
            dcache_store_update_data.eq(mem_shifted_store_data),
            dcache_fill_valid.eq(0),
            dcache_fill_addr.eq(memory.addr),
            dcache_fill_data.eq(self.lsu.response_load_value),
            dcache_flush_all.eq(0),
            dcache_invalidate_valid.eq(0),
            dcache_invalidate_addr.eq(memory.addr),
            line_match.eq(self.frontend.line_valid & ((memory.addr & Const(0xFFFFFFFFFFFFFFF8, 64)) == self.frontend.line_base)),
            ...
            cached_memory_next_pc.eq(
                Mux(
                    ...
                    Mux(memory.reg_write & (memory.reg_index == 15), dcache_load_result, memory.next_pc),
                )
            ),
                ...
                with m.Case(offset):
                    if offset == 0:
                        m.d.comb += [
                            ...
                            mem_shifted_store_data.eq(memory.store_value),
                            dcache_shifted_read.eq(dcache_lookup_data),
                        ]
                    else:
                        m.d.comb += [
                            mem_shifted_sel.eq(mem_base_sel << offset),
                            mem_shifted_store_data.eq(memory.store_value << (offset * 8)),
                            dcache_shifted_read.eq(Cat(dcache_lookup_data[offset * 8:64], Const(0, offset * 8))),
                        ]

        m.d.comb += dcache_load_result.eq(Mux(memory.width_bytes == 1, dcache_shifted_read & 0xFF,
                                          Mux(memory.width_bytes == 2, dcache_shifted_read & 0xFFFF,
                                          Mux(memory.width_bytes == 4, dcache_shifted_read & 0xFFFFFFFF,
                                          dcache_shifted_read))))
        ...
        with m.If(memory.post_reg_write):
            with m.If(memory.reg_write):
                with m.If(memory.post_reg_index == memory.reg_index):
                    m.d.comb += [
                        cached_memory_reg_write.eq(1),
                        cached_memory_reg_index.eq(memory.reg_index),
                ...
                with m.Else():
                    m.d.comb += [
                        ...
                        cached_memory_aux_reg_index.eq(memory.post_reg_index),
                        cached_memory_aux_reg_value.eq(cached_memory_post_value),
                    ]
            with m.Else():
                m.d.comb += [
                    cached_memory_reg_write.eq(1),
                    cached_memory_reg_index.eq(memory.post_reg_index),
                    cached_memory_reg_value.eq(cached_memory_post_value),
                ]
        with m.Else():
            m.d.comb += [
                cached_memory_reg_write.eq(memory.reg_write),
                cached_memory_reg_index.eq(memory.reg_index),
                cached_memory_reg_value.eq(dcache_load_result),
            ]
        ...
        with m.If(self.halted):
            m.d.comb += next_state.eq(V3PipelineState.HALTED)
        with m.Elif(self.locked_up | fetch_error | paging_without_mmu | memory_error):
            m.d.comb += next_state.eq(V3PipelineState.LOCKED_UP)
        with m.Elif(walk_active):
            m.d.comb += next_state.eq(Mux(walk_processing, V3PipelineState.WALK_PROCESS, V3PipelineState.WALK))
        with m.Elif(memory.valid & ~memory.phys_valid):
            m.d.comb += next_state.eq(V3PipelineState.MEM_TRANSLATE)
        with m.Elif(~fetch_phys_valid):
            m.d.comb += next_state.eq(V3PipelineState.FETCH_TRANSLATE)
        with m.Elif(vector_load_active & ~vector_phys_valid):
            m.d.comb += next_state.eq(V3PipelineState.INTERRUPT_VECTOR_TRANSLATE)
        with m.Elif(vector_load_active):
            m.d.comb += next_state.eq(V3PipelineState.VECTOR_LOAD)
        with m.Elif(retire.valid):
            m.d.comb += next_state.eq(V3PipelineState.RETIRE)
        with m.Elif(memory.valid):
            m.d.comb += next_state.eq(V3PipelineState.MEMORY)
        with m.Elif(execute_valid):
            m.d.comb += next_state.eq(V3PipelineState.EXECUTE)
        with m.Elif(decode_valid):
            m.d.comb += next_state.eq(V3PipelineState.DECODE)
        ...
        with m.Else():
            ...
            with m.If(self.halted | self.locked_up):
                m.d.sync += clear_inflight_sync() + [
                    walk_active.eq(0),
                    walk_processing.eq(0),
                    walk_started.eq(0),
                ]
            with m.Elif(fetch_error | paging_without_mmu | memory_error):
                m.d.sync += clear_inflight_sync() + clear_instruction_latches_sync() + [
                    walk_active.eq(0),
                    walk_processing.eq(0),
                    walk_started.eq(0),
                    self.locked_up.eq(1),
                ]
            with m.Elif(vector_translate_lockup | walk_fault_lockup):
                m.d.sync += clear_inflight_sync() + clear_instruction_latches_sync() + [
                    walk_active.eq(0),
                    walk_processing.eq(0),
                    walk_started.eq(0),
            ...
            with m.Elif(vector_translate_complete):
                m.d.sync += [
                    vector_load_addr.eq(vector_translate_phys),
                    vector_phys_valid.eq(1),
                ]
            with m.Elif(memory_translate_complete):
                m.d.sync += [
                    memory.addr.eq(memory_translate_phys),
                    memory.phys_valid.eq(1),
                ]
            with m.Elif(fetch_translate_complete):
                m.d.sync += [
                    self.fetch_phys_addr.eq(fetch_translate_phys),
                    fetch_phys_valid.eq(1),
                ]
            with m.Elif(vector_translate_ready & paging_enabled & ~vector_translate_lockup & ~vector_translate_complete):
                m.d.sync += [
                    walk_active.eq(1),
                    walk_processing.eq(0),
                    walk_started.eq(0),
            ...
            with m.Elif(memory_translate_ready & paging_enabled & ~memory_fault & ~memory_translate_complete):
                m.d.sync += [
                    walk_active.eq(1),
                    walk_processing.eq(0),
                    walk_started.eq(0),
                    walk_virtual_addr.eq(memory.virtual_addr),
                    walk_table_addr.eq(self.special_regs.page_table_root_physical),
                    walk_level.eq(2),
                    walk_access.eq(Mux(memory.write, ACCESS_WRITE, ACCESS_READ)),
                    walk_resume_kind.eq(WALK_RESUME_MEMORY),
                    walk_pte_latched.eq(0),
                ]
            with m.Elif(fetch_translate_ready & paging_enabled & ~fetch_fault & ~fetch_translate_complete):
                m.d.sync += [
                    walk_active.eq(1),
                    walk_processing.eq(0),
                    walk_started.eq(0),
            ...
            with m.Elif(trap_preempt_lockup):
                m.d.sync += clear_inflight_sync() + clear_instruction_latches_sync() + [
                    walk_active.eq(0),
                    walk_processing.eq(0),
                    walk_started.eq(0),
            ...
            with m.Elif(walk_response_valid):
                with m.If(walk_response_error):
                    m.d.sync += clear_inflight_sync() + clear_instruction_latches_sync() + [
                        walk_active.eq(0),
                        walk_processing.eq(0),
                        walk_started.eq(0),
            ...
            with m.Elif(vector_response_valid):
                with m.If(vector_response_error | (vector_handler_pc == 0)):
                    m.d.sync += clear_inflight_sync() + clear_instruction_latches_sync() + [
                        vector_load_active.eq(0),
                        vector_load_started.eq(0),
                        vector_virtual_addr.eq(0),
                ...
                with m.Else():
                    m.d.sync += clear_retire_stage_sync() + [
                        vector_load_active.eq(0),
                        vector_load_started.eq(0),
                        vector_virtual_addr.eq(0),
                        vector_load_addr.eq(0),
            ...
            with m.Elif(retire_stage.halt_request | retire_stage.lockup_request):
                m.d.sync += clear_inflight_sync() + clear_instruction_latches_sync()
            with m.Else():
                m.d.sync += clear_retire_stage_sync()
                ...
                with m.If(memory_to_retire):
                    ...
                    with m.If(~cached_memory_response & memory.write & mem_single_line_access):
                    ...
                    with m.If(~cached_memory_response & memory.write & line_match):
                    ...
                    with m.If(memory.set_reservation):
                        m.d.sync += [
                            ll_reservation_addr.eq(memory.reservation_addr),
                ...
                with m.Elif(execute_to_retire):
                    m.d.sync += [
                        ...
                        retire.commit_pc.eq(self.execute_pc),
                ...
                with m.Elif(memory_to_retire | memory_error):
                    m.d.sync += clear_memory_stage_sync() + [
                ...
                with m.Elif(execute_to_memory):
                    m.d.sync += [
                        memory.valid.eq(1),
                        memory.request_started.eq(0),
                        memory.virtual_addr.eq(execute_stage.outputs.memory_addr),
                        memory.addr.eq(execute_stage.outputs.memory_addr),
                        memory.phys_valid.eq(~paging_enabled),
                        memory.width_bytes.eq(execute_stage.outputs.memory_width_bytes),
                        memory.write.eq(execute_stage.outputs.memory_write),
                        memory.store_value.eq(execute_stage.outputs.memory_store_value),
                        memory.reg_write.eq(execute_stage.outputs.memory_reg_write),
                        memory.reg_index.eq(execute_stage.outputs.memory_reg_index),
                        memory.next_pc.eq(execute_stage.outputs.memory_next_pc),
                        memory.commit_pc.eq(self.execute_pc),
                        memory.fault_pc.eq(self.execute_pc),
                        memory.flags_write.eq(execute_stage.outputs.memory_flags_write),
                        memory.flags_value.eq(execute_stage.outputs.memory_flags_value),
                        memory.set_reservation.eq(execute_stage.outputs.memory_set_reservation),
                        memory.reservation_addr.eq(execute_stage.outputs.memory_reservation_addr),
                        memory.post_reg_write.eq(execute_stage.outputs.memory_post_reg_write),
                        memory.post_reg_index.eq(execute_stage.outputs.memory_post_reg_index),
                        memory.post_reg_value.eq(execute_stage.outputs.memory_post_reg_value),
                        memory.post_reg_delta.eq(execute_stage.outputs.memory_post_reg_delta),
                        memory.post_reg_use_load_result.eq(execute_stage.outputs.memory_post_reg_use_load_result),
                        memory.chain_store.eq(execute_stage.outputs.memory_chain_store),
                        memory.chain_store_addr.eq(execute_stage.outputs.memory_chain_store_addr),
                        memory.chain_store_use_load_result.eq(execute_stage.outputs.memory_chain_store_use_load_result),
                        memory.chain_store_value.eq(execute_stage.outputs.memory_chain_store_value),
                ...
                with m.Elif(memory.valid & memory_stage.start_request):
                    m.d.sync += memory.request_started.eq(1)
                ...
                with m.Elif(execute_valid & (execute_to_memory | execute_to_retire)):
                    m.d.sync += [
                        execute_valid.eq(0),
                        self.execute_instruction.eq(0),
                    ]
...
__all__ = ['Little64V3Core', 'V3PipelineState']
