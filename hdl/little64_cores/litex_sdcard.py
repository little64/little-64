...
from migen import ClockDomain, ClockSignal, Instance, Memory, Module, Record, ResetSignal, Signal


EMULATOR_VERILOG_FILENAMES = (
    "sd_common.v",
    "sd_const.vh",
    "sd_link.v",
    "sd_params.vh",
...
def _resolve_emulator_verilog_dir() -> Path:
    candidates: list[Path] = []

    try:
        from litesdcard.emulator.core import __file__ as emulator_core_file

        candidates.append(Path(emulator_core_file).resolve().parent / "verilog")
    except ImportError:
        pass

    candidates.append(Path(__file__).resolve().parent / "litesdcard_emulator_verilog")

    for candidate in candidates:
        if all((candidate / filename).is_file() for filename in EMULATOR_VERILOG_FILENAMES):
            return candidate

    searched = ", ".join(str(candidate) for candidate in candidates)
    raise FileNotFoundError(
        f"LiteSDCard emulator Verilog sources are missing; searched: {searched}"
    )


def sd_emulator_pads() -> Record:
    return Record([
        ("clk", 1),
        ("cmd_i", 1),
        ("cmd_o", 1),
...
class Little64SDEmulator(Module):
    def __init__(self, platform) -> None:
        self.pads = pads = sd_emulator_pads()

        self.clock_domains.cd_sd_ll = ClockDomain("sd_ll", reset_less=True)
        self.comb += self.cd_sd_ll.clk.eq(pads.clk)

        self.specials.buffer = Memory(32, 512 // 4, init=[0 for _ in range(512 // 4)])
        ...
        self.specials.host_wr_port = self.buffer.get_port(write_capable=True)
        ...
        self.host_buffer_write_addr = Signal(7)
        self.host_buffer_write_data = Signal(32)
        self.host_buffer_write_enable = Signal()

        self.comb += [
            self.host_wr_port.adr.eq(self.host_buffer_write_addr),
            self.host_wr_port.dat_w.eq(self.host_buffer_write_data),
            self.host_wr_port.we.eq(self.host_buffer_write_enable),
        ]
        ...
        platform.add_verilog_include_path(os.fspath(verilog_dir))
        platform.add_sources(os.fspath(verilog_dir), "sd_common.v", "sd_link.v", "sd_phy.v")


class Little64SDCardImageBridge(Module):
    def __init__(self, platform, emulator: Little64SDEmulator) -> None:
        pads = platform.request("sdcard_img")
        self.comb += [
            pads.req.eq(emulator.block_read_act),
            pads.byteaddr.eq(emulator.block_read_byteaddr),
            emulator.host_buffer_write_addr.eq(pads.write_addr),
            emulator.host_buffer_write_data.eq(pads.write_data),
            emulator.host_buffer_write_enable.eq(pads.write_enable),
            emulator.block_read_go.eq(pads.done),
            emulator.block_write_done.eq(emulator.block_write_act),
        ]
