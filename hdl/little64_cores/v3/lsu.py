...
class V3LSUState(IntEnum):
    IDLE = 0
    FIRST = 1
    SECOND = 2


class Little64V3LSU(Elaboratable):
    def __init__(self,
                 *,
        ...
        self.state = Signal(2, init=V3LSUState.IDLE)
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += [
            bus_active.eq(self.state != V3LSUState.IDLE),
        ...
        m.d.comb += [
            self.request_ready.eq(self.state == V3LSUState.IDLE),
            self.active.eq(self.state != V3LSUState.IDLE),
            ...
            self.bus.adr.eq(Mux(self.state == V3LSUState.SECOND, next_word_addr, word_aligned_addr)),
        ]
        ...
        m.d.comb += [
            ...
            self.bus.dat_w.eq(Mux(self.state == V3LSUState.SECOND, second_beat_dat_w, first_beat_dat_w)),
            self.bus.sel.eq(Mux(self.state == V3LSUState.SECOND, second_beat_sel, first_beat_sel)),
            self.bus.cyc.eq(self.state != V3LSUState.IDLE),
            self.bus.stb.eq(self.state != V3LSUState.IDLE),
            self.bus.we.eq(latched_write & (self.state != V3LSUState.IDLE)),
            self.bus.cti.eq(0),
            self.bus.bte.eq(0),
        ]
        ...
        with m.If((self.state == V3LSUState.IDLE) & self.request_valid):
            m.d.sync += [
                ...
                beat_started.eq(0),
                self.state.eq(V3LSUState.FIRST),
            ]
        with m.Elif(self.state == V3LSUState.FIRST):
            ...
            with m.Elif(self.bus.ack):
                ...
                with m.Else():
                    ...
                    m.d.sync += self.state.eq(V3LSUState.IDLE)
        with m.Elif(self.state == V3LSUState.SECOND):
            with m.If(~beat_started):
                m.d.sync += beat_started.eq(1)
            with m.Elif(effective_err):
                m.d.comb += [self.response_valid.eq(1), self.response_error.eq(1)]
...
__all__ = ['Little64V3LSU', 'V3LSUState']