...
class Little64V3LineCache(Elaboratable):
    def __init__(self, *, entries: int = 4, address_width: int = 64, data_width: int = 64) -> None:
        if entries < 1 or entries & (entries - 1):
            raise ValueError('Little64V3LineCache entries must be a power of two and at least 1')
        if data_width != 64:
            raise ValueError('Little64V3LineCache currently expects 64-bit cache lines')
